...
function updateHeatmap(month)
{
  ...
  // console.log(markers[month][0].lat());
  if(heatmapLayer)
  {
    heatmapLayer.setMap(null);
  }
  document.getElementById("output2").innerHTML = "Month displaying: "+months[month];
  heatmapLayer = new google.maps.visualization.HeatmapLayer({data: locations[month]});
  heatmapLayer.setMap(heatmap);
}

function initMap() {
  map = new google.maps.Map(document.getElementById("map"), {zoom:4, center: center,mapTypeId: 'roadmap'});
  heatmap = new google.maps.Map(document.getElementById("heatmap"),{zoom:4, center: center,mapTypeId: 'satellite'});
  // var markers = []
  // var markers = {}
