...
class UtilConvertSchemaTestCase(TestCase):
    ...
    def test_type_one(self):
        ...
        data = data_type_one(file)
        convert_schema(config, file_path, 1)
        added = {
            "latitude": [],
            "longitude": [],
            "time": [],
    ...
    def test_convert_type_two(self):
        ...
        data = data_type_two(file)
        convert_schema(config, file_path, 2)
    ...
    def test_convert_type_three(self):
        ...
        data = data_type_three(file)
        convert_schema(config, file_path, 3)
        assert len(data) >= len(Data.objects.filter(name=config))
        count = 0
        for row in data:
